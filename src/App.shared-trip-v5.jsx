...
function ProfileDataEditor({ profile, onSave, onClose }) {
  ...
  return <FullScreenOverlay onClose={onClose}><SheetHead title="Данные путешественника" onClose={onClose}/>
    <div style={{fontSize:11.5,color:T.subd,lineHeight:1.45,marginBottom:14}}>TripWise использует профиль для автоподстановки и определения, нужны ли международные документы. Паспортные данные и ответы визовых анкет в MVP остаются только на этом устройстве.</div>
...
function tripTimeline(t) {
  ...
  for(const [key,type,icon] of [["activities","activity","📍"],["custom","prep","✓"]])for(const x of (t[key]||[]))if(x.startDate)add({id:`${type}-${x.id}`,type,date:x.startDate,time:x.startTime||"",endTime:x.endTime||"",title:`${icon} ${x.name}`,sub:x.location||x.notes||"",location:x.location||"",raw:x});
...
function SharedTripScreen({ t, initialBlk, onBack, onUpdate, onDelete, onLeaveTrip, onFindTickets, goHotels, goDocs, setToast, onReplaceTrip, onUndoable, syncState="saved", bottomStr = "0px" }) {
  ...
  return <div style={{ padding: "12px 14px", paddingBottom: 150, animation: "slideIn .18s ease-out" }}>
    ...
    {mode === "trip" ? <>
      ...
      <div style={{display:"flex",gap:8,marginTop:10}}><div onClick={()=>setImportOpen(true)} className="press" style={{flex:1,textAlign:"center",border:`1px solid ${T.violet}55`,background:T.violet+"12",borderRadius:13,padding:11,color:T.violet,fontSize:11.5,fontWeight:800,cursor:"pointer"}}>✦ Импорт брони</div><div onClick={()=>setPeopleOpen(true)} className="press" style={{flex:1,textAlign:"center",border:`1px solid ${T.line}`,background:T.card,borderRadius:13,padding:11,color:T.text,fontSize:11.5,fontWeight:800,cursor:"pointer"}}>👥 Участники</div></div>
    </> : <>
...
export default function App() {
  ...
  const profileServerView=v=>({fullName:v&&v.fullName||"",homeCity:v&&v.homeCity||"",homeCountry:v&&v.homeCountry||"",homeAirport:v&&v.homeAirport||"",defaultCurrency:v&&v.defaultCurrency||"EUR"});
  useEffect(()=>{let dead=false;(async()=>{const local=store.get("profile",{})||{},r=await sharedApi("profile-get",{},12000);if(!dead&&r.ok&&r.profile){const merged={...r.profile,...local};setProfile(merged);store.set("profile",merged);}})();return()=>{dead=true;};},[]);
  const saveProfile=async(v)=>{const next={...(v||{})};setProfile(next);store.set("profile",next);const r=await sharedApi("profile-save",{profile:profileServerView(next)},12000);return r;};
  ...
  return <div style={{ minHeight: "100vh", background: T.bg, display: "flex", justifyContent: "center" }}>
    ...
    <div className="app-root" style={{ width: "100%", maxWidth: 420, paddingTop: safeTop, background: `radial-gradient(105% 54% at 78% 0%, #0d1830 0%, ${HOME_T.bg} 48%, ${HOME_T.bgDeep} 100%)`, color: tab === "home" ? HOME_T.text : T.text, fontFamily: "Manrope,sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {invitePreview&&<Overlay onClose={()=>setInvitePreview(null)}><SheetHead title="Приглашение в поездку" onClose={()=>setInvitePreview(null)}/><div style={{background:T.card,border:`1px solid ${T.line}`,borderRadius:17,padding:14}}><div style={{fontSize:10.5,color:T.violet,fontWeight:800,marginBottom:5}}>{invitePreview.creatorName} приглашает вас</div><div style={{fontFamily:"Sora,sans-serif",fontSize:18,fontWeight:800,color:T.text}}>{invitePreview.title}</div><div style={{fontSize:12,color:T.subd,marginTop:5}}>{[invitePreview.destination,invitePreview.df&&invitePreview.dt?`${ddmm(invitePreview.df)} — ${ddmm(invitePreview.dt)}`:"",`${invitePreview.travelerCount} ${plural(invitePreview.travelerCount,"путешественник","путешественника","путешественников")}`].filter(Boolean).join(" · ")}</div></div><div style={{display:"flex",gap:9,marginTop:12}}><div onClick={()=>setInvitePreview(null)} className="press" style={{flex:1,textAlign:"center",border:`1px solid ${T.line}`,borderRadius:13,padding:12,color:T.sub,fontWeight:800,cursor:"pointer"}}>Позже</div><div onClick={async()=>{const p=invitePreview,r=await sharedApi("join-trip",{tripId:p.tripId,code:p.code});if(r.ok&&r.trip){setTrips(xs=>xs.some(x=>x.id===r.trip.id)?xs.map(x=>x.id===r.trip.id?r.trip:x):[r.trip,...xs]);setInvitePreview(null);setTripSection("overview");setTripOpen(r.trip.id);setTab("routes");setStack(["trip"]);setToast("Вы присоединились к поездке");}else setToast(r.error==="trip full"?"В поездке уже максимум участников":"Не удалось присоединиться");}} className="press" style={{flex:1.4,textAlign:"center",background:GRAD.cta,borderRadius:13,padding:12,color:"#fff",fontWeight:900,cursor:"pointer"}}>Присоединиться</div></div></Overlay>}
