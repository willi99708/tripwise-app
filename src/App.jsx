...
function stripServerFields(trip) {
  if (!trip) return trip;
  const { members, askGroup, creatorId, shareCode, schemaVersion, updatedAt, _viewer, activityLog, travelerStates, settlementPayments, ...core } = trip;
  return core;
}
...
function tripProgress(t) {
  ...
  if (b.tickets) items.push(["tickets", !!checks.tickets && hasFlight]);
  if (b.lodging && !t.lodgingOff) {
    if ((t.stays || []).length) for (const s of t.stays) items.push(["stay:" + s.id, !!s.done]);
    else {
...
function nextAction(t) {
  ...
  if (b.lodging && !t.lodgingOff) {
    ...
    if (!(t.stays||[]).length && (!checks.lodgeMain || (t.route && t.route.stopover && !checks.lodgeStop))) return { block: "lodging", title: "Подтвердите жильё", sub: "закройте все ночёвки поездки", btn: "К отелям", act: "hotels", tone: T.violet };
  }
...
function tripTimeline(t) {
  ...
  for(const x of (t.stays||[])){if(x.startDate)add({id:`stay-in-${x.id}`,type:"stay",date:x.startDate,time:x.startTime||"15:00",title:`🏠 Заезд · ${x.name}`,sub:x.location||"",location:x.location||x.name,raw:x});if(x.endDate)add({id:`stay-out-${x.id}`,type:"stay",date:x.endDate,time:x.endTime||"11:00",title:`🏠 Выезд · ${x.name}`,sub:x.location||"",location:x.location||x.name,raw:x});}
  for(const [key,type,icon] of [["transport","transport","🚗"],["activities","activity","📍"],["custom","prep","✓"]])for(const x of (t[key]||[]))if(x.startDate)add({id:`${type}-${x.id}`,type,date:x.startDate,time:x.startTime||"",endTime:x.endTime||"",title:`${icon} ${x.name}`,sub:x.location||x.notes||"",location:x.location||"",raw:x});
...
function travelerReadiness(t,tr,budget) {
  ...
  const items=[];
  for(const doc of docs){const done=stateDocs[doc.id]!==undefined?!!stateDocs[doc.id]:!!globalDocs[doc.id];items.push({id:`doc:${doc.id}`,label:doc.name,done,kind:"doc"});}
...
function tripTodayState(t) {
  const today=iso(new Date()),active=!!t.df&&today>=String(t.df).slice(0,10)&&today<=String(t.dt||t.df).slice(0,10),timeline=tripTimeline(t),todayEvents=timeline.filter((x)=>x.date===today);
  const nowHM=`${pad(new Date().getHours())}:${pad(new Date().getMinutes())}`; const next=todayEvents.find((x)=>!x.time||x.time>=nowHM)||null;
  return{today,active,timeline,todayEvents,next};
}
...
function SharedTripScreen({ t, initialBlk, onBack, onUpdate, onDelete, onLeaveTrip, onFindTickets, goHotels, goDocs, setToast, onReplaceTrip, bottomStr = "0px" }) {
  ...
  const docsDone = docs.filter((x) => docChecks[x.id]).length;
  const prepItems = [
    ...(t.servicesAdded || []).map((id) => ({ id: "svc:" + id, name: (EXTRA_SERVICES.find((x) => x.id === id) || {}).title || id, done: !!svcChecks[id], kind: "svc", raw: id })),
    ...(t.custom || []).map((x) => ({ id: "custom:" + x.id, name: x.name, done: !!x.done, kind: "custom", raw: x.id, item: x })),
  ];
  ...
  const addTraveler = () => { const name = travelerName.trim() || `Путешественник ${travelers.length + 1}`; upd((x) => { const cur = activeTravelers(x); return { ...x, travelers: [...(x.travelers || []), { id: "tr" + Date.now(), name, memberId: null, active: true, createdAt: new Date().toISOString() }], travelerTarget: cur.length + 1 }; }); setTravelerName(""); };
  const removeTraveler = (tr) => { if (tr.memberId) { setToast("Сначала удалите пользователя из поездки — его финансовый слот останется отдельно"); return; } upd((x) => { const cur = activeTravelers(x); return { ...x, travelers: (x.travelers || []).map((y) => y.id === tr.id ? { ...y, active: false } : y), travelerTarget: Math.max(1, cur.length - 1) }; }); };
  const saveTitle = () => { const title = renameValue.trim(); if (!title) { setToast("Название не может быть пустым"); return; } upd((x) => ({ ...x, title: title.slice(0, 80) })); setSettingsOpen(false); setToast("Название обновлено"); };
  ...
  const markSettlement = async (s) => { setSettleBusy(true); const r = await sharedApi("record-settlement", { tripId: t.id, fromTravelerId: s.from, toTravelerId: s.to, amount: s.amount, currency: s.currency || "EUR" }); setSettleBusy(false); if (r.ok && r.trip) { replace(r.trip); setToast("Перевод учтён"); } else setToast("Не удалось учесть перевод"); };
  ...
  const refreshFx = async () => { if (!isCreator) return; const quotes = [...new Set(budget.groups.map((g) => g.currency).filter((x) => x !== baseDraft))]; setFxBusy(true); const r = await sharedApi("fx-rates", { tripId: t.id, base: baseDraft, quotes }); setFxBusy(false); if (r.ok) { upd((x) => ({ ...x, baseCurrency: baseDraft, fxSnapshot: { base: r.base || baseDraft, date: r.date || iso(new Date()), rates: r.rates || {}, source: r.source || "Frankfurter", capturedAt: new Date().toISOString() } })); setToast("Курсы зафиксированы для бюджета"); } else setToast("Не удалось получить курсы"); };

  const addJourneyLeg = () => setJourneyLegs((a) => [...a, { flightNumber: "", date: a[a.length - 1]?.date || t.df || "" }]);
  const enrichLegs = async (rows) => { const legs = []; let missing = 0, providerMissing = 0; for (const row of rows) { const r = await sharedApi("flight-info", { tripId: t.id, flightNumber: row.flightNumber, date: row.date }); if (!r.found) missing++; if (r.providerConfigured === false) providerMissing++; legs.push({ ...((r && r.flight) || { flightNumber: prettyFlightNumber(row.flightNumber) }), date: row.date }); } return { legs, missing, providerMissing }; };
  const addFlightJourney = async () => { const rows = journeyLegs.map((x) => ({ flightNumber: String(x.flightNumber || "").trim().toUpperCase(), date: x.date || "" })).filter((x) => x.flightNumber); if (!rows.length) { setToast("Введите хотя бы один номер рейса"); return; } const { legs, missing, providerMissing } = await enrichLegs(rows); const first = legs[0], last = legs[legs.length - 1]; const name = (first.fromCode && last.toCode) ? `${first.fromCode} → ${last.toCode}` : `Перелёт ${prettyFlightNumber(first.flightNumber)}`; const j = { id: "fj" + Date.now(), name, legs, done: true, priceAmount: Number(journeyCost.priceAmount) || null, currency: journeyCost.currency || "EUR", pricingMode: journeyCost.pricingMode || "total", splitTravelerIds: (journeyCost.splitTravelerIds || []).length ? journeyCost.splitTravelerIds : travelers.map((x) => x.id), payments: normalizePayments(journeyCost), createdAt: new Date().toISOString() }; upd((x) => ({ ...x, flightJourneys: [...(x.flightJourneys || []), j], baseCurrency: x.baseCurrency || j.currency })); setFlightOpen(false); setJourneyLegs([{ flightNumber: "", date: t.df || "" }]); setJourneyCost(emptyCost()); if (!missing) setToast(legs.length > 1 ? "Перелёт с пересадкой добавлен" : "Рейс найден и добавлен"); else if (providerMissing) setToast("Перелёт добавлен; live-данные появятся после подключения flight API"); else setToast("Часть рейсов не найдена — сохранены вручную"); };
  ...
  const askAI = async (preset) => { const q = String(preset || chatText || "").trim(); if (!q || chatBusy) { if (preset) setChatOpen(true); return; } setChatOpen(true); setChatText(""); setMessages((m) => [...m, { role: "user", text: q }]); setChatBusy(true); const r = await sharedApi("trip-ai", { tripId: t.id, question: q }); setChatBusy(false); setMessages((m) => [...m, { role: "assistant", text: r.ok ? (r.answer || "Не нашёл ответа в данных поездки.") : "Не удалось связаться с TripWise AI." }]); };
  ...
  const applyImport = async () => {
    ...
    else if (b.kind === "activity") upd((x) => ({ ...x, activities: [...(x.activities || []), common] }));
  ...
  return <div style={{ padding: "12px 14px", paddingBottom: 150, animation: "slideIn .18s ease-out" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}><div onClick={onBack} className="press" style={{ width: 34, height: 34, borderRadius: 11, border: `1px solid ${T.line}`, display: "grid", placeItems: "center", cursor: "pointer" }}><Icon d={I.back} size={16} color={T.text} /></div><div style={{ flex: 1 }} /><div onClick={() => setPeopleOpen(true)} className="press" style={{ display: "flex", alignItems: "center", cursor: "pointer" }}>{members.slice(0, 4).map((m, i) => <div key={m.id || i} style={{ width: 30, height: 30, borderRadius: 999, marginLeft: i ? -7 : 0, border: `2px solid ${T.bg}`, background: gradFor((m.displayName || "X").slice(0, 2)), display: "grid", placeItems: "center", fontSize: 9.5, fontWeight: 800, color: "#fff", overflow: "hidden" }}>{m.photoUrl ? <img src={m.photoUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : memberInitials(m)}</div>)}</div>{isCreator && <div onClick={invite} className="press" style={{ height: 30, borderRadius: 999, border: `1px solid ${T.violet}66`, background: T.violet + "1e", padding: "0 10px", display: "grid", placeItems: "center", color: T.violet, fontSize: 12, fontWeight: 800, cursor: "pointer" }}>＋ Invite</div>}<div onClick={() => { setRenameValue(t.title || ""); setSettingsOpen(true); }} className="press" style={{ width: 30, height: 30, borderRadius: 999, border: `1px solid ${T.line}`, display: "grid", placeItems: "center", color: T.subd, fontSize: 18, cursor: "pointer" }}>⋯</div></div>

    <div style={{ position: "relative", borderRadius: 22, overflow: "hidden", background: gradFor(t.dc), padding: 16, minHeight: 116, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}><div style={{ position: "absolute", inset: 0, background: "linear-gradient(transparent,rgba(5,5,20,.78))" }} /><div style={{ position: "relative" }}><div style={{ fontFamily: "Sora,sans-serif", fontWeight: 800, fontSize: 20, color: "#fff" }}>{t.title}</div><div style={{ fontSize: 12, color: "rgba(255,255,255,.84)", marginTop: 3 }}>{[t.dcName, t.country].filter(Boolean).join(", ")}{t.df ? ` · ${fmtShort(new Date(t.df))}` : ""}{t.dt ? ` — ${fmtShort(new Date(t.dt))}` : ""}{today.active ? " · сейчас в поездке" : d != null ? ` · через ${d} дн.` : ""} · {travelers.length} {plural(travelers.length, "человек", "человека", "человек")}</div></div></div>
    ...
    {mode === "trip" ? <>
      ...
      <div style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 18, padding: "0 12px", marginBottom: 10 }}>
        <SectionHead k="tickets" icon="✈️" title="Билеты" sub={flightCount ? `${flightCount} ${plural(flightCount, "рейс", "рейса", "рейсов")}${checks.tickets ? " · куплены" : " · требуют подтверждения"}` : "Рейсы ещё не добавлены"} done={checks.tickets ? 1 : 0} total={1} action={() => setFlightOpen(true)} />
        {open.tickets && <div style={{ padding: "0 0 12px 44px" }}>{routeSegs.length ? <div style={{ marginBottom: 8 }}><div style={{ fontSize: 10.5, color: T.subd, marginBottom: 5 }}>Маршрут TripWise</div><JourneyCard j={{ name: tripCodes(t.route) || "Выбранный маршрут", legs: routeSegs }} /></div> : null}{manualJourneys.map((j) => <JourneyCard key={j.id} j={j} />)}{legacyFlights.map((s, i) => <JourneyCard key={s.id || i} j={s} legacy />)}{!flightCount && <Empty>Добавьте прямой рейс или весь перелёт с пересадками.</Empty>}{isCreator && <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>{!t.route && <div onClick={() => onFindTickets(t)} className="press" style={{ fontSize: 11.5, fontWeight: 800, color: T.violet, cursor: "pointer" }}>Найти билеты</div>}<div onClick={() => upd((x) => ({ ...x, checks: { ...x.checks, tickets: !x.checks.tickets } }))} className="press" style={{ fontSize: 11.5, fontWeight: 800, color: checks.tickets ? T.green : T.subd, cursor: "pointer" }}>{checks.tickets ? "✓ Билеты куплены" : "Отметить билеты купленными"}</div></div>}</div>}
        <SectionHead k="lodging" icon="🏠" title="Жильё" sub={lodgeTotal ? `${lodgeDone} из ${lodgeTotal} подтверждено` : "Добавьте бронирования жилья"} done={lodgeDone} total={lodgeTotal} action={() => startAdd("stays")} />
      ...
      {!isCreator && <div style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 14, padding: 11, fontSize: 11.5, color: T.subd, marginTop: 9 }}>План общий. Изменения фиксирует создатель; предложения выносите в <b style={{ color: T.text }}>Ask Group</b>. Личные документы и взаиморасчёты можно отмечать самому.</div>}
    </> : <>
...
export default function App() {
  ...
  const [notifyPrefs, setNotifyPrefs] = useState(() => store.get("notifyPrefs", { deals: true, promos: true, deadlines: true, group: true, changes: true, news: true }));
