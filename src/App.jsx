...
function SharedTripScreen({ t, initialBlk, onBack, onUpdate, onDelete, onLeaveTrip, onFindTickets, goHotels, goDocs, setToast, onReplaceTrip, bottomStr = "0px" }) {
  ...
  const askAI = async (preset) => {
    const q = String(preset || chatText || "").trim(); if (!q || chatBusy) { if (preset) setChatOpen(true); return; }
    setChatOpen(true); setChatText(""); setMessages((m) => [...m, { role: "user", text: q }]); setChatBusy(true);
    try {
      const r = await sharedApi("trip-ai", { tripId: t.id, question: q }, 35000);
      const text = r.ok ? (r.answer || "Не нашёл ответа в данных поездки.") : (r.error === "timeout" ? "TripWise AI не ответил за 35 секунд. Попробуйте ещё раз — запрос больше не будет висеть бесконечно." : `TripWise AI сейчас недоступен${r.error ? ` · ${r.error}` : ""}.`);
      setMessages((m) => [...m, { role: "assistant", text }]);
    } catch (e) { setMessages((m) => [...m, { role: "assistant", text: "TripWise AI сейчас недоступен." }]); }
